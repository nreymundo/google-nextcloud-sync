...
_TOKEN_RE = re.compile(
    r"(?P<key>(?i)(?:access|refresh|id|auth)(?:[_\- ]?)token)\s*[:=]\s*(?P<val>[A-Za-z0-9\-_\.]{10,})"
)
...
def mask_pii(text: str) -> str:
    """Mask PII in freeform text: emails, phones, and token-like sequences."""
    if not text:
        return text
            ...
            if hasattr(record, "__dict__"):
                for k in list(self.EXTRA_KEYS_TO_MASK):
                    if k in record.__dict__ and isinstance(record.__dict__[k], str):
                        record.__dict__[k] = mask_pii(record.__dict__[k])
        except Exception:
            pass
        return True
...
class JsonFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:  # type: ignore[override]
        base: dict[str, Any] = {
            ...
            "name": record.name,
            "msg": mask_pii(self._format_message(record)),
        }
